...
class handDetector():
    ...
    def findPosition(self, img, handNo=0, draw=True):
        xList = []    #all value of x will put here
        yList = []
        bbox = []
        self.lmList = []
        if self.results.multi_hand_landmarks:
            ...
            xmin, xmax = min(xList), max(xList)                  #making rectangular box around the hand
            ymin, ymax = min(yList), max(yList)
            bbox = xmin, ymin, xmax, ymax

            if draw:
                cv2.rectangle(img, (xmin - 20, ymin - 20), (xmax + 20, ymax + 20),   #making rectangular box around the hand
                              (0, 255, 0), 2)
